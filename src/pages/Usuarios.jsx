...
export default function Usuarios() {
  ...
  useEffect(() => {
    ...
    // Filtramos según rol
    if (user.role === "admin") {
      setUsuarios(allUsers); // Admin ve todos
    } else if (user.role === "empresa") {
      setUsuarios(allUsers.filter(u => u.empresaId === user.empresaId));
    } else {
      setUsuarios([]); // Cliente u otros roles podrían ver nada o lo suyo
    }
  }, []);

  return (
    <div className="flex min-h-screen">
      <Sidebar isCollapsed={false} setIsCollapsed={() => {}} isOpen={false} setIsOpen={() => {}} />
            ...
            <tbody>
              {usuarios.map(u => (
                <tr key={u.id}>
                  ...
                  <td className="border-b p-2">{u.nombre}</td>
                  <td className="border-b p-2">{u.empresaId}</td>
                </tr>
              ))}
            </tbody>
          </table>
